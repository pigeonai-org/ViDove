...
class GptVisionAgent(VisionAgent):
    def __init__(self, model_name, model_path, frame_per_seg, cache_dir=None):
        ...
        self.client = OpenAI()
        print("GPT Vision Agent initialized.")
    ...
    def summarize_cue(self):
        ...
        response = openai.chat.completions.create(
            ...
            messages=[
                ...
                The list consists of image descriptions of {self.frame_per_seg} frames from a video.\
                The list will be provided by chronological order. You should extract the most important details and in your final return.\
                You can also provide a list the objects in the image with the reorganized response if you think it is important."},
                {
                    "role": "user", 
                    "content": prompt
                }
        ...
        self.log(f"Summary response: {response.choices[0].message.content}")
        return response.choices[0].message.content

    def analyze_video(self, video_path):
        self.log(f"Analysing video file using GPT: {video_path}")
...
#GptVisionAgent("openai/clip-vit-base-prompt", "openai/clip-vit-base-prompt", 4, None).analyze_video("test5.mp4")
...
class assistant_vision_api(VisionAgent):
    ...
    def analyze_video(self, video_path):
        self.log(f"Analysing video file using GPT agent: {video_path}")
        cap = cv2.VideoCapture(video_path)
        total_frames = int(cap.get(cv2.CAP_PROP_FRAME_COUNT))
    ...
    def cleanup(self):
        self.log(f"Cleansing the thread and uploaded files.")
        while self.file_ids:
...
class CLIPVisionAgent(VisionAgent):
    ...
    def analyze_video(self, video_path):
        ...
        self.log(f"Analysing video file using CLIPS: {video_path}")
...
class HfVisionAgent:
    ...
    def load_model(self):
        ...
        try:
            ...
            if self.verbose:
                print(f"Failed to load model locally. Attempting to download from Hugging Face: {e}")
            model = AutoModelForCausalLM.from_pretrained(self.model_name, device_map="auto")
            tokenizer = AutoTokenizer.from_pretrained(self.model_name)
            self.pipe = pipeline("image-text-to-text", model=model, tokenizer=tokenizer, device=self.device)
    ...
    def analyze_video(self, video_path):
        ...
        self.log(f"Analysing video file using HFmodel {self.model_name}: {video_path}")
